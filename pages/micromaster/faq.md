...
- **Q3:** [What are the hours of the internship?](#Q3:_What_are_the_hours_of_the_internship?)
- **Q4:** [Is this a paid internship?](#Q4:_Is_this_a_paid_internship?)
- **Q5:** [What happens after the initial steps?](#Q5:_What_happens_after_the_initial_steps?)
- **Q6:** [If I'm an international student, how soon can I get my paperwork required by my visa?](#Q6:_If_I'm_an_international_student,_how_soon_can_I_get_my_paperwork_required_by_my_visa?)
- **Q7:** [How many pull requests and issues in total do I need to make to finish the steps?](#Q7:_How_many_pull_requests_and_issues_in_total_do_I_need_to_make_to_finish_the_steps?)
- **Q8:** [How long do I have to complete these steps?](#Q8:_How_long_do_I_have_to_complete_these_steps?)
- **Q9:** [How can I keep track of 10 steps vetting progress?](#Q9:_How_can_I_keep_track_of_10_steps_vetting_progress?)
...
- **Q19:** [What if I accidentally resign my manager account, or delete my planet folder, how do I recreate my manager profile?](#Q19:_What_if_I_accidentally_resign_my_manager_account,_or_delete_my_planet_folder,_how_do_I_recreate_my_manager_profile?)
- **Q20:** [What do I do if I get the error message "A Virtual Box machine with the name 'prod' already exists"?](#Q20:_What_do_I_do_if_I_get_the_error_message_"A_Virtual_Box_machine_with_the_name_'prod'_already_exists"?)
...
+ This is an intensive internship that requires 16 hours of work each week. Work with your fellow interns and keep us updated in the [Slack chat](http://slack.ole.org).
...
+ You can keep track of your progress using the [Tracking Progress](https://planetlearning.github.io/#!./pages/micromaster/trackprogress.md) link. 
...
+ You should first check if Vagrant Cloud is up and running by looking at [HashiCorp's status page](https://status.hashicorp.com):
  ...
  ![Vagrant Cloud Status](images/vagrant-cloud-status.png)
  ...
  2. Click on the last version's (the uppermost) version number.
    ...
    ![Box Version](images/vagrant-cloud-last-version-box.png)
  ...
  4. After you download the box, run the following commands while in your `planet` directory. Also, include the correct path to the box you just downloaded:

        ``` bash
        vagrant box add treehouses/buster64 /path/to/vagrant-box.box
        vagrant init treehouses/buster64
        vagrant up
        ```
...
+ You have a few options:
  ...
  2. If you are not using the repo anymore you can delete it and then follow the [First Steps](firststeps.md).
...
+ Running two communities on the same machine is possible as long as their configurations (VirtualBox name, port forwarding, etc.) does not collide with each other.
  ...
  2. If you will not run those two communities at the same time:
    - clone `planet` again and name the folder `planet-1`:

      ```
      git clone https://github.com/open-learning-exchange/planet.git planet-1
      cd planet-1
      ```

     - change line 25 of the `vagrant` file
...
+ Note: Before trying the steps below, make sure your planet is up to date.
  ...
  ```
  Please follow the instructions [here](https://planetlearning.github.io/#!./pages/micromaster/planet-installation-vagrant.md) to reinstall planet. (**Note**: After planet reinstallation, you will need to register as admin a second time as you did in step [Planet Configurations](https://planetlearning.github.io/#!./pages/micromaster/configurations-vagrant.md). However, you cannot use the same "Name" in Set up Configuration step because that "Name" already exists. You may instead use YourName01 or add any other number you like at the end of your original "Name". This should allow you to register as the admin and log in your community planet.)
